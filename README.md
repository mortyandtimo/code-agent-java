...
这个项目不是把大模型包一层 HTTP 接口，而是尝试用 `Spring Boot + MySQL + DDD` 去复现一个最小但完整的 coding agent：任务如何创建、上下文如何累积、工具如何执行、失败如何恢复、会话如何继续。

我做这个项目的出发点很直接：想把对 Agent Loop 的理解，从“会调用 API”推进到“能把一个 agent 当作后端系统来设计和迭代”。

## Why

很多 agent demo 的重点是“模型能不能调工具”，但真正落地时，问题往往不在单次调用，而在循环本身：
...
所以这个项目的目标一直不是“再做一个聊天后端”，而是做一个可以运行、可以恢复、可以审计、可以继续迭代的 code agent skeleton。
...
我在迭代里越来越确信，系统不能在工具失败之后重新做一轮大范围探索，而应该围绕“刚才到底哪里失败了”去恢复。所以现在的 loop 里会根据上一次工具失败内容，生成针对性的 recovery prompt，而不是让模型重新开始摸索。
...
这个项目把危险动作确认放在了工具执行边界：

- 危险 shell 命令先拦截
- 单文件删除走 `delete_file` 专用工具，而不是放任模型自由拼 shell
- CLI 中明确做用户确认

我的判断是：提示词可以降低风险，但真正可靠的边界一定要落在 executor 层。

### 5. 长上下文当然重要，但上下文“结构正确”更重要

我在做恢复、流式输出、工具回填这些环节时一个很深的体会是：agent 不是单纯拼更多 token 就更强。

如果历史消息结构错了，或者工具消息缺字段，模型即使能吃很长上下文，也会直接在协议层失败。相比“盲目堆上下文”，我更重视：
...
项目整体按 DDD 组织，不把 agent 逻辑散落在 controller 或 CLI 里。
...
这里用 DDD，不是为了追求术语，而是因为 agent 天然就适合按“任务状态机 + 会话状态 + 事件记录”去建模。这样后面要扩工具、扩 provider、做任务恢复或者审计，结构都比较稳。
...
这个项目不是一开始就长成现在这样，很多设计判断都是在处理真实问题时逐步收敛出来的。几个比较关键的点：
...
这些点也决定了我现在看 agent 项目时，更关注 loop 稳定性、恢复能力和执行边界，而不是单纯追求“看起来更聪明”。
